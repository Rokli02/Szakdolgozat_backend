...
public interface UserRepository extends PagingAndSortingRepository<User, Long> {
  ...
  @Query(value = "SELECT * FROM user u INNER JOIN role r ON r.id = u.role_id WHERE u.name LIKE %:filter% OR u.birthdate LIKE %:filter% OR u.username LIKE %:filter% OR u.email LIKE %:filter% OR r.name LIKE %:filter% GROUP BY username", countQuery = "SELECT count(*) FROM user u INNER JOIN role r ON r.id = u.role_id  WHERE u.name LIKE %:filter% OR u.birthdate LIKE %:filter% OR u.username LIKE %:filter% OR u.email LIKE %:filter% OR r.name LIKE %:filter% GROUP BY username", nativeQuery = true)
